...
class FichaController extends Controller
{
    ...
    public function store(Request $request)
    {
        try
        {
            ...
            $requ=\Validator::make($request->all(), [
                'nume_ficha'                    => 'required|max:7|unique:tf_fichas_individuales',
    ...
    public function fichaBienescomunes(Ficha $ficha){
        ...
        $logos=Institucion::first();
        $lote=$ficha->id_lote;
        $total=fichaIndividual::whereHas('fichas', function($query) use ($lote) {
    ...
    public function fichaInformativa(Ficha $ficha){
        ...
        $html = $html->render();
        $mpdf->setFooter('<div style="background-color: #4646A3;border: solid 1px #fff !important;color:#fff;margin:0;padding:0;line-height:14px;text-align: center">LA INFORMACION CONTENIDA EN EL PRESENTE NO GENERA NI RECONOCE DERECHOS DE PROPIEDAD</div><div style="border: solid 1px #fff !important;text-align: center; font-size:8px;">MEJORAMIENTO E IMPLEMENTACIÓN DE LA CAPACIDAD OPERATIVA DE LA OFICINA DE CATASTRO URBANO Y RURAL EN EL DISTRITO DE SAN SEBASTIAN- CUSCO</div>');
        $mpdf->WriteHTML($html);
        $mpdf->Output($fileName,'I');

    }

    public function fichaNumeracion(Ficha $ficha){
        $mytime= Carbon::now('America/Lima');
        ...
        $fecha=date("d/m/Y", strtotime($mytime));
        $hora=date("H:m:s", strtotime($mytime));
        $html = \View::make('pages.pdf.numeracion',compact('ficha','logos','fecha','hora'));
        $html = $html->render();
        $mpdf->setFooter('<div style="background-color: #4646A3;border: solid 1px #fff !important;color:#fff;margin:0;padding:0;line-height:14px;text-align: center">LA INFORMACION CONTENIDA EN EL PRESENTE NO GENERA NI RECONOCE DERECHOS DE PROPIEDAD</div><div style="border: solid 1px #fff !important;text-align: center; font-size:8px;">MEJORAMIENTO E IMPLEMENTACIÓN DE LA CAPACIDAD OPERATIVA DE LA OFICINA DE CATASTRO URBANO Y RURAL EN EL DISTRITO DE SAN SEBASTIAN- CUSCO</div>');
        $mpdf->WriteHTML($html);
        $mpdf->Output($fileName,'I');
    ...
    public function certificadocatastral(Ficha $ficha){
        $mytime= Carbon::now('America/Lima');
        ...
        $fecha=date("d/m/Y", strtotime($mytime));
        $hora=date("H:m:s", strtotime($mytime));
        $html = \View::make('pages.pdf.certificadocatastral',compact('ficha','logos','fecha','hora'));
        $html = $html->render();
        $mpdf->setFooter('<div style="background-color: #4646A3;border: solid 1px #fff !important;color:#fff;margin:0;padding:0;line-height:14px;text-align: center">LA INFORMACION CONTENIDA EN EL PRESENTE NO GENERA NI RECONOCE DERECHOS DE PROPIEDAD</div><div style="border: solid 1px #fff !important;text-align: center; font-size:8px;">MEJORAMIENTO E IMPLEMENTACIÓN DE LA CAPACIDAD OPERATIVA DE LA OFICINA DE CATASTRO URBANO Y RURAL EN EL DISTRITO DE SAN SEBASTIAN- CUSCO</div>');
        $mpdf->WriteHTML($html);
        $mpdf->Output($fileName,'I');
